...
def main():
    ...
    model_path = os.path.join(folder_path, 'model', 'model_axial_fine-{epoch:02d}-{val_binary_accuracy:.2f}.hdf5')
    ...
    with strategy.scope():
        model = model_axial_fine((256, 256, 7), 7)
        model.compile(optimizer=Adam(epsilon=1e-8),
                      loss='binary_crossentropy',
                      metrics=['binary_accuracy'])
    lr_sched = step_decay_schedule(initial_lr=0.008, decay_factor=0.9, step_size=8)
    ...
    plt.savefig(os.path.join(folder_path, "history.png"))
    plt.close()
